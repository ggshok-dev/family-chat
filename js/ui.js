...
function showMessageMenu(event, msg) {
  ...
  if (isTextMessage && msg.from === currentUser && allButtons[index]) {
    allButtons[index].addEventListener('click', function() {
      ...
      if (newText && newText !== msg.text) {
        db.ref(getChatPath() + '/' + msg.id).update({text: newText, edited: true});
      }
    });
    index++;
  }
  ...
  // Удалить
  if ((msg.from === currentUser || hasPermission('canDeleteOtherMessages')) && allButtons[index]) {
    allButtons[index].addEventListener('click', function() {
      menu.remove();
...
function switchTheme() {
  const currentIndex = THEMES.findIndex(function(t) { return t.id === currentTheme || t.class === currentTheme; });
  ...
  if (themeBtn) themeBtn.textContent = theme.emoji;
}
...
function applyStoredSettings() {
  ...
  document.body.classList.remove('dark-theme', 'green-theme', 'purple-theme');
  if (savedTheme === 'dark-theme') document.body.classList.add('dark-theme');
  ...
  if (themeBtn) {
    ...
    themeBtn.textContent = theme ? theme.emoji : '🌊';
  }
}
...
function saveAvatar(userId, dataUrl) {
  ...
  db.ref('users/' + userId + '/avatar').set(dataUrl);
  renderUsers();
}

function resetAvatar(userId) {
  localStorage.removeItem('fc_av_' + userId);
  db.ref('users/' + userId + '/avatar').remove();
  renderUsers();
}
...
function setupUIListeners() {
  ...
  document.getElementById('msgInput').addEventListener('keydown', function(e) {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      sendText(this.value, replyToMessage);
    }
  });
  ...
  document.addEventListener('click', function(e) {
    ...
    if (pk && eb && !pk.contains(e.target) && e.target !== eb) {
...
document.getElementById('soundToggle').addEventListener('change', function() {
  ...
  document.getElementById('fontUp').addEventListener('click', function() {
    if (fontSize < 150) { fontSize += 10; updateFontSize(); }
  });
  
  document.getElementById('fontDown').addEventListener('click', function() {
    if (fontSize > 80) { fontSize -= 10; updateFontSize(); }
  });
